...
object SaveFinalBuyDataToAnDa {    //提取hive内的数据，生成DataFrame.   输入日期段，把安达  A表写入B表。
   val logger = Logger.getLogger(SaveFinalBuyDataToAnDa.getClass)

  def main(args: Array[String]): Unit = {
    val conf = new SparkConf().setAppName("etl anda history of data")
//  .setMaster("local[5]")
...
//    HiveTableUtil.dropBuyDataOfPartition(hiveContext, "ba_model", "sale_transaction_detail", partitions)
    ...
    val dr_shop_df = hiveContext.table("ba_model.dim_shop")   // 通过零售商店铺code,获取DR店铺编码。
